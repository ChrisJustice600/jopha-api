const updateMasterPackStatus = async (req, res) => {
    ...
    try {
      ...
      if (!masterPack || masterPack.groupageId !== groupage.id) {
        return res.status(404).json({ error: "MasterPack non trouvé ou n'appartient pas au groupage" });
      }
      ...
      console.error("Erreur lors de la mise à jour du statut du MasterPack :", error);
